<template>
  <div class="restaurant-container">
    ...
    <canvas id="turntable" width="1000" height="1000">当前浏览器暂不支持canvas</canvas>
  </div>
</template>
<script>
/**
...
class Turntable {
  ...
  _paintArrow () {
    ...
    this.context.lineTo(this.clientX, 280);
    this.context.lineTo(this.clientX - 30, this.clientY);
    this.context.fill();
  }
...
export default {
  ...
  mounted () {
    this.turntableClass = new Turntable(document.getElementById('turntable'), restautantMap,  (item) => {
      this.$message({
        message: `${item.name}, 走起`,
        type: 'success'
      });
    });
